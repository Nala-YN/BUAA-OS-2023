	...
	// Step 1: Ensure free page
		if (LIST_EMPTY(&page_free_swapable_list)) {
				...
				for(i=0;i<=1023;i++){
					Pde* pg=pgdir+i;
					for(j=0;j<=1023;j++){
						Pte* pte=(Pte*)KADDR(PTE_ADDR(*pg));
						pte=pte+j;
						if((*pte&PTE_V)&&PTE_ADDR(*pte)==0x3900000){
							*pte=*pte&(~PTE_V);
							tlb_invalidate(asid,KADDR(0x3900000));
							*pte=*pte|PTE_SWP;
							*pte=*pte&0x00000fff;
							*pte=*pte|PTE_ADDR(da);
				...
				memcpy(da,(void*)KADDR(0x3900000),BY2PG);
				LIST_INSERT_HEAD(&(page_free_swapable_list),pp,pp_link);
				return pp;
					}
...
static int is_swapped(Pde *pgdir, u_long va) {
	...
	static void swap(Pde *pgdir, u_int asid, u_long va) {
		...
		Pte* pte0;
		pgdir_walk(pgdir,va,0,&pte0);
		u_char* da=(u_char*)PTE_ADDR(*pte0);
		memcpy((void*)KADDR(page2pa(pp)),(void*)da,BY2PG);
		int i,j;
		for(i=0;i<=1023;i++){
			Pde* pg=pgdir+i;
			for(j=0;j<=1023;j++){
				Pte* pte=(Pte*)KADDR(PTE_ADDR(*pg));
				pte=pte+j;
				if((*pte&PTE_SWP)&&!(*pte&PTE_V)&&(PTE_ADDR(*pte)==da)){
				*pte=(*pte|PTE_V)&!PTE_SWP;
				tlb_invalidate(asid,KADDR(PTE_ADDR(*pte)));
				*pte=*pte&0x00000fff;
				*pte=*pte|(page2pa(pp));
			}
		}
